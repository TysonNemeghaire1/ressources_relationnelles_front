  ...
  return (
    <Link href={`ressource/${data.id}`}>
      <div className={`${isTrendy ? "" : "flex flex-col gap-3"} w-full p-2 hover:bg-custom-blue-0`}>
        <div
          className={`${isTrendy ? "flex" : "flex flex-col gap-3"} w-full justify-between`}
        >
          <p
            className={`${isTrendy ? "w-1/2" : "w-full"} text-black font-bold text-lg`}
          >
            {data.name}
          </p>
          <div
            className={`${isTrendy ? "w-1/2" : "w-full"} text-right italic font-light`}
          >
            <p>
